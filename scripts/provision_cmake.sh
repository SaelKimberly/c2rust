...
if type "cmake" >/dev/null 2>&1; then
  ...
  cmake --version | grep -q "3.9.1" && {
    echo "Cmake requirements already met. Nothing to do."
    exit 0
  }
fi
...
curl -s -O $CMAKE_URL || {
  echo >&2 "Cmake download failed. Aborting."
  exit 1
}

# check integrity of the downloaded script
echo $CMAKE_SHA256 >cmake_sha256sum.txt
sha256sum -c cmake_sha256sum.txt 2>&1 | grep OK || {
  echo >&2 "Checksum mismatch. Aborting."
  exit 1
}

chmod u+x $CMAKE_SCRIPT
mkdir -p $CMAKE_PREFIX
./$CMAKE_SCRIPT --prefix=$CMAKE_PREFIX --skip-license || {
  echo >&2 "Cmake installation failed. Aborting."
  exit 1
}
