...
def validate_csmith_home() -> None:
    ...
    if not os.access(csmith_header, os.R_OK):
        ...
        print(
            "Please set the CSMITH_HOME environment variable to the "
...
def create_compile_commands(dirname: str, output_c_name: str) -> str:
    ...
    compile_commands_settings = [
        {
            "directory": dirname,
            "arguments": [C_COMPILER, "-I", CSMITH_HOME, output_c_name],
            "file": output_c_name,
        }
    ]

    compile_commands_name = os.path.join(dirname, "compile_commands.json")
    with open(compile_commands_name, "w", encoding="utf-8") as filename:
...
def generate_c_source(dirname: str, output_c_name: str) -> None:
    ...
    with open(output_c_name, "w", encoding="utf-8") as output_c:
...
def compile_c_file(output_c_name: str, output_c_exe_name: str) -> None:
    ...
    subprocess.run(
        compile_cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
...
def execute_driver(exe_name: str) -> bytes:
    ...
    exec_result = subprocess.run(
        exe_name, capture_output=True, check=True, timeout=CSMITH_TIMEOUT
    )
...
def compile_rust_file(
    output_c_name: str, output_rs_name: str, output_rs_exec_name: str
) -> None:
